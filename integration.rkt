 ...
 #|
     Prints beliefs [s s^] for all nodes in lst
 |#
 ...
 |#
 (define (concept-belief-calc lst neg?)
   (cond
 ...
 (define (edge-belief-calc pubmed-counts neg?)
  ...
  #| Given a single list of predicates refering to A->B split it into two A->X, X->B for 2-Hop Queries |#
  ...
  (define (edge->bms-node edge bms) 
    ...
    (define subject-belief (concept-belief-calc (for/list ((x (publications-info-alist-from-edge edge))) (list-ref x 2)) (negated? edge)))
    (define object-belief (concept-belief-calc (for/list ((x (publications-info-alist-from-edge edge))) (list-ref x 3))  (negated? edge)))
  ...
  (define (bms-add-node curieA curieB predicates bms)
    ...
    (for ((edge (edges/query query 'A->B)))
      (begin 
        ...
        (define subject_score (concept-belief-calc (for/list ((x (publications-info-alist-from-edge edge))) (list-ref x 2)) (negated? edge))) 

        (define object_score (concept-belief-calc (for/list ((x (publications-info-alist-from-edge edge))) (list-ref x 3))  (negated? edge)))
        ...
        (define -> (tms-create-node bms '-> #:belief edge_score)) 
        (justify-node (gensym) A->B (list A B ->) prior)
